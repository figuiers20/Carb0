    ...
    try:
        session_obj = stripe.checkout.Session.create(
            ...
            shipping_address_collection={
                "allowed_countries": [
                    "NL", "BE", "DE", "FR", "AT", "IT", "ES", "PT",
                    "IE", "LU", "FI", "DK", "SE", "PL", "CZ",
                ],
            },
            line_items=[{
                "price_data": {
                    ...
                    "product_data": {
                        ...
                        "images": [f"{BASE_URL}/static/carb0-box.png"],
                    },
                },
                "quantity": quantity,
            }],
...
def order_success():
    ...
    if session_id:
        try:
            ...
            shipping = checkout.shipping_details
            order_info = {
                "email": checkout.customer_details.email if checkout.customer_details else None,
                "amount": f"€{checkout.amount_total / 100:.2f}" if checkout.amount_total else None,
                "sachet_size": checkout.metadata.get("sachet_size", "") if checkout.metadata else "",
                "quantity": checkout.metadata.get("quantity", "") if checkout.metadata else "",
                "ship_to": f"{shipping.name}, {shipping.address.city}, {shipping.address.country}" if shipping else None,
            }
        except Exception:
            pass
